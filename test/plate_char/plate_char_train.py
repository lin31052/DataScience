...
log(f"训练 {len(train_ds)} / 验证 {len(val_ds)}")
...
plt.tight_layout(); plt.savefig(f"{OUT}/char_03_metrics_zh.png", dpi=110); plt.close()
...
plt.tight_layout(); plt.savefig(f"{OUT}/char_04_confusion.png", dpi=110); plt.close()
...
plt.tight_layout(); plt.savefig(f"{OUT}/char_05_predictions.png", dpi=110); plt.close()

with open(f"{OUT}/char_result.json", "w") as f:
